...
RebPolygon::RebPolygon(void) {
   m_pPoints = NULL;
   m_pIndis  = NULL;
   m_NumP    = 0;
   m_NumI    = 0;
   m_Flag    = 0;
   memset(&m_Aabb, 0, sizeof(RebAabb));
   } // constructor
/*----------------------------------------------------------------*/
...
void RebPolygon::Set(const RebVector *pPoints, int nNumP,
   ...
   m_pPoints = new RebVector[nNumP];
   m_pIndis  = new unsigned int[nNumI];

   m_NumP = nNumP;
   m_NumI = nNumI;

   memcpy(m_pPoints, pPoints, sizeof(RebVector)*nNumP);
...
void RebPolygon::CalcBoundingBox(void) {
   RebVector vcMax, vcMin;
...
void RebPolygon::SwapFaces(void) {
...
void RebPolygon::Clip(const RebPlane &Plane, RebPolygon *pFront, 
                      RebPolygon *pBack) {
   if (!pFront && !pBack) return;

   RebVector vcHit, vcA, vcB;
   RebRay    Ray;

   // cast away const
   RebPlane  *pPlane = ((RebPlane*)&Plane);
   ...
   RebVector *pvcFront = new RebVector[m_NumP*3];
   RebVector *pvcBack  = new RebVector[m_NumP*3];
...
void RebPolygon::Clip(const RebAabb &aabb) {
   RebPolygon BackPoly, ClippedPoly;
   RebPlane   Planes[6];
   bool       bClipped=false;

   // cast away const
   RebAabb *pAabb = ((RebAabb*)&aabb);
   ...
   for (int i=0; i<6; i++) {
      if ( Planes[i].Classify(ClippedPoly) == RebCLIPPED ) {
...
int RebPolygon::Cull(const RebAabb &aabb) {
   RebPlane Planes[6];
   int      nClass=0;
   ...
   for (int p=0; p<6; p++) {
      ...
      if (bFirst) {
         ...
         if (nInside  == m_NumP) return RebVISIBLE;
         }
...
void RebPolygon::CopyOf(const RebPolygon &Poly) {
...
bool RebPolygon::Intersects(const RebRay &Ray, bool bCull, float *t) {
   // cast away const
   RebRay *pRay = (RebRay*)&Ray;
...
bool RebPolygon::Intersects(const RebRay &Ray, bool bCull, float fL, float *t) {
   // cast away const
   RebRay *pRay = (RebRay*)&Ray;
